...
export const StadiumMap: React.FC<StadiumMapProps> = ({
  ...
  return (
    <div className="glass-panel rounded-xl p-4 overflow-hidden bg-surface border border-outline-variant/30 flex flex-col" id="stadium-map-card">
      ...
      <div className="relative w-full aspect-video min-h-[340px] max-h-[420px] bg-surface-dim rounded-lg border border-outline-variant/40 flex items-center justify-center p-2 overflow-hidden shadow-inner">
        {/* SVG Map Layout */}
        <svg viewBox="0 0 800 450" className="w-full h-full select-none">
          <defs>
            <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M 40 0 L 0 0 0 40" fill="none" stroke="var(--outline-variant)" strokeWidth="0.5" strokeOpacity="0.7" />
            </pattern>
          ...
          />
          <text x="412" y="70" fill="var(--on-surface)" className="font-mono text-[9px] tracking-wide font-bold pointer-events-none">
          ...
          <rect
            ...
            strokeWidth="2.5"
            className="cursor-pointer hover:fill-surface-container-high transition-all"
            onClick={() => {
          ...
          />
          <text x="400" y="335" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[11px] font-bold pointer-events-none">
            Parking Lots A, B, C
          </text>
          ...
          />
          <text x="665" y="295" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[11px] font-bold pointer-events-none">
          ...
          >
            <circle cx="150" cy="120" r="32" fill="var(--surface)" stroke={getStatusColor(transit.find(t => t.id === "rail_transit")?.status || "HEAVY")} strokeWidth="3" />
            <text x="150" y="118" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[10px] font-bold">
              RAIL
            </text>
          ...
          >
            <circle cx="650" cy="110" r="32" fill="var(--surface)" stroke={getStatusColor(transit.find(t => t.id === "shuttle_bus")?.status || "MODERATE")} strokeWidth="3" />
            <text x="650" y="108" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[10px] font-bold">
              SHUTTLE
            </text>
          ...
          <ellipse cx="400" cy="220" rx="140" ry="100" fill="var(--surface)" stroke="var(--outline)" strokeWidth="4" />
          
          {/* Inner Seating Area */}
          <ellipse cx="400" cy="220" rx="100" ry="70" fill="var(--surface-container-low)" stroke="var(--outline-variant)" strokeWidth="2" />
          ...
          {/* Stadium Label */}
          <text x="400" y="174" textAnchor="middle" fill="var(--on-surface)" className="font-sans font-bold text-[10px] tracking-wider" fillOpacity="0.6">
          ...
          >
            ...
            <circle cx="400" cy="113" r="10" fill="var(--surface)" stroke={getStatusColor(gates.find(g => g.id === "gate_a")?.status || "LOW")} strokeWidth="2" />
            <text x="400" y="116" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[8px] font-bold">A</text>
          </g>
          ...
          >
            ...
            <circle cx="548" cy="220" r="10" fill="var(--surface)" stroke={getStatusColor(gates.find(g => g.id === "gate_b")?.status || "LOW")} strokeWidth="2" />
            <text x="548" y="223" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[8px] font-bold">B</text>
          </g>
          ...
          >
            ...
            <circle cx="400" cy="327" r="10" fill="var(--surface)" stroke={getStatusColor(gates.find(g => g.id === "gate_c")?.status || "LOW")} strokeWidth="2" />
            <text x="400" y="330" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[8px] font-bold">C</text>
          </g>
          ...
          >
            ...
            <circle cx="252" cy="220" r="10" fill="var(--surface)" stroke={getStatusColor(gates.find(g => g.id === "gate_d")?.status || "LOW")} strokeWidth="2" />
            <text x="252" y="223" textAnchor="middle" fill="var(--on-surface)" className="font-sans text-[8px] font-bold">D</text>
          </g>
          ...
          >
            <circle cx="295" cy="145" r="10" fill="var(--surface)" stroke={getStatusColor(gates.find(g => g.id === "gate_e")?.status || "LOW")} strokeWidth="2" />
            <text x="295" y="148" textAnchor="middle" fill="var(--primary)" className="font-sans text-[8px] font-bold">♿</text>
          </g>


          {/* PEDESTRIAN CONCOURSE FLOW PATH LINES */}
          <path d="M 150 152 L 250 220" stroke="var(--status-alert)" strokeWidth="2" strokeDasharray="5, 5" fill="none" strokeOpacity="0.5" />
          <path d="M 618 110 L 548 220" stroke="var(--status-go)" strokeWidth="2" strokeDasharray="5, 5" fill="none" strokeOpacity="0.5" />
          <path d="M 400 300 L 400 317" stroke="var(--status-critical)" strokeWidth="3" strokeDasharray="4, 4" fill="none" strokeOpacity="0.5" />
          ...
          {incidents.filter(inc => !inc.resolved).map((inc) => {
            ...
            return (
              ...
              >
                <circle cx={x} cy={y} r="18" fill="none" stroke={inc.severity === "CRITICAL" ? "var(--status-critical)" : "var(--status-alert)"} strokeWidth="2.5" strokeOpacity="0.8">
                ...
                <circle cx={x} cy={y} r="6" fill={inc.severity === "CRITICAL" ? "var(--status-critical)" : "var(--status-alert)"} />
              </g>
            );
          })}
        </svg>
        ...
        {selectedAssetId && (
          <div className="absolute bottom-3 left-3 right-3 bg-surface/95 backdrop-blur border border-primary/20 rounded-lg p-3 text-xs flex items-center justify-between shadow-xl animate-fade-in">
            <div>
            ...
            <button
              ...
              className="text-on-surface-variant hover:text-primary font-mono text-[10px] bg-surface-container hover:bg-surface-container-highest px-2 py-1 rounded cursor-pointer transition-colors"
            >
              Clear
            </button>
          </div>
